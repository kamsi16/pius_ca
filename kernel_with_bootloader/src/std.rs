...
pub fn input_str() -> Option<String> {
    ...
    while character != Some('\u{000D}') && character != Some('\u{000A}') {
        //Test for all three breakout conditions
        match character {
            None => {
                //do nothing
            }
            ...
            Some('\u{000A}') => {
                //escape pressed. Return None from the function immediately
                *KEY_PRESSED.lock() = None; //clear global KEY_PRESSED so that effect is not repeated
                return None;
            }
            Some('\u{000D}') => {
                //Simply breakout of loop if carriage return is pressed.
                *KEY_PRESSED.lock() = None; //clear global KEY_PRESSED so that effect is not repeated
                break;
            }
            _ => {
                //Every other unicode key sent, push to input
